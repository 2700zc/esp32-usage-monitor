...
#if BOARD_BTN_THIRD
// BOOT key (GPIO0 on 2.16, rightmost button). Tracks its own state so
// main.cpp can send a custom BLE command on short press.
static void scanBootKey() {
  ...
  if (s_boot.wasPressed) s_boot.pressedAt = millis();
